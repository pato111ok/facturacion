...
$id_proforma= $_SESSION['id_proforma'];
$numero_proforma= $_SESSION['numero_proforma'];
...
$insert_tmp=mysqli_query($con, "INSERT INTO detalle_proforma (numero_proforma, id_producto,cantidad,precio_venta) VALUES ('$numero_proforma','$id','$cantidad','$precio_venta')");
...
$sql=mysqli_query($con, "select * from products, detalle_proforma where products.id_producto=detalle_proforma.id_producto and detalle_proforma.id_detalle='".$id_detalle."'");
	while ($row=mysqli_fetch_array($sql))
	{
	...
	$id_detalle=intval($_GET['id']);	
	$delete=mysqli_query($con, "DELETE FROM detalle_proforma WHERE id_detalle='".$id_detalle."'");
}
...
<?php
	...
	$sql=mysqli_query($con, "select * from products, proformas, detalle_proforma where proformas.numero_proforma=detalle_proforma.numero_proforma and  proformas.id_proforma='$id_proforma' and products.id_producto=detalle_proforma.id_producto");
	while ($row=mysqli_fetch_array($sql))
	{
	...
	$update=mysqli_query($con,"update proformas set total_venta='$total_factura' where id_proforma='$id_proforma'");
?>
<tr>
