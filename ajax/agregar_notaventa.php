...
if (!empty($id) and !empty($cantidad) and !empty($precio_venta))
{
	...
	$sql=mysqli_query($con,"SELECT combo_producto, cant_combo_producto FROM products where id_producto='".$id."'");
...
<?php
	...
	while ($row=mysqli_fetch_array($sql))
	{
		/*if( $row['cantidad_tmp']<=$row["cantidad_producto"] ){*/
			...
			if($iva_producto!=0){
				...
				<tr>
					...
					<td><?php if($iva_producto!=1){ echo $nombre_producto;}else{echo "* ".$nombre_producto;}?></td>
...
<tr>
	...
	<td class='text-right' id="total_factura"><?php echo number_format($total_factura,2);?></td>
	<td></td>
</tr>
